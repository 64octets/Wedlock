...
function wedlock_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'wedlock' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'wedlock' ),
...
function wedlock_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && wedlock_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'wedlock' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( esc_html__( 'Leave a comment', 'wedlock' ), esc_html__( '1 Comment', 'wedlock' ), esc_html__( '% Comments', 'wedlock' ) );
		echo '</span>';
	}
